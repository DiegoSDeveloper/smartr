...
export const Util = {
  ...
  getAlignClassName: function (
    type?: AlignType,
    defaultValue: AlignType = AlignType.START
  ): string | null {
  ...
  getDefaultValue: function (type?: ColumnType) {
    if (type) {
      switch (type) {
  ...
  isMinOrDefaultValue: function (value: any, type?: ColumnType) {
    if (type) {
      switch (type) {
        case ColumnType.FLOAT:
        case ColumnType.INT:
  ...
  getColumnValue: function (column: TableColumnProps, record: any): any {
  ...
  getFormattedDate(dateString, timeZone, culture) {
    ...
    const adjustedDateString =
      dateString.length === 10 ? `${dateString}T12:00:00Z` : dateString;
  ...
  getFormattedDateTime(dateString, timeZone, culture) {
    ...
    const date = new Date(
      dateString.length === 10 ? dateString : `${dateString}Z`
    );
    if (isNaN(date.getTime())) return "";
